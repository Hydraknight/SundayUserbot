...
if Var.TG_BOT_USER_NAME_BF_HER is not None and tgbot is not None:
    ...
    async def inline_handler(event):
        ...
        result = None
        query = event.text
        if event.query.user_id == bot.uid and query.startswith("My"):
            rev_text = query[::-1]
        ...
        if event.query.user_id == bot.uid and query == "stats":
            result = builder.article(
                title="Stats",
                text=
                f"**Showing Stats For {DEFAULTUSER}** \nNote --> Only Owner Can Check This \n(C) @imDivu",
                buttons=[
                    [custom.Button.inline("Show Stats ", data="terminator")],
                    [Button.url("Hinata", "https://t.me/MissHinata_Bot")],
    ...
    async def rip(event):
        ...
        await tgbot.send_message(
            LOG_CHAT,
            message=
            f"Hello, A [Nigga](tg://user?id={him_id}). Wants To Talk With You.",
    ...
    async def rip(event):
        ...
        await tgbot.send_message(
            LOG_CHAT,
            message=
            f"Hello, A [Nigga](tg://user?id={him_id}). Wants To Ask You Something.",
            buttons=[Button.url("Contact Him", f"tg://user?id={him_id}")],
        )
